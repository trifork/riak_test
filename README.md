...
**Note:** To build `riak_ee` instead of OS `riak`, set
[`$RT_REPO`](https://github.com/basho/riak_test/blob/master/bin/rtdev-build-releases.sh#L11).
to be `riak_ee` instead of the default `riak`.
...
of `meck` needs to be updated, too.  This is handled by the script.
...
the repo name, e.g. `riak_ee-2.0.0`.  To use `riak_ee` set [`$RT_USE_EE`](https://github.com/basho/riak_test/blob/master/bin/rtdev-setup-releases.sh#L23)
